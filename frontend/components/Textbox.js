...
const Textbox = ({submit,func,loading,pics}) => {
  return (
    <form className = "text-container" onSubmit = {submit}>
        ...
        <textarea className = {loading && "loading-text"}id = {pics.length === 0 ? "textbox-nothing" : "textbox"} placeholder="Type in details about yourself" onChange={(e) => func(e.target.value)}/>
