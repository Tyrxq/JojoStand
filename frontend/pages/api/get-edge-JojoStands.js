...
async function generateFetch(userDescription, standPicture) {
    const payload = {
      model: "gpt-3.5-turbo",
      messages: [
        {
    ...
    return { text: lookForJson(responseData.choices[0].message.content), pics: response_pic };
};

export default async function handler(req){
    const json = await req.json();
    const response = await generateFetch(json.prompt,predictFetch);
